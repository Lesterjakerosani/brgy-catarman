      ...
      { accessorKey: "status", header: "Status", cell: ({ row }) => <StatusBadge status={row.original.status} /> },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1">
            ...
            <RowActions
              actions={[
                { label: "Print Case Record", icon: Printer, onClick: () => router.push(`/dashboard/blotters/${row.original.id}/print`) },
                ...
                { label: "Archive Case", icon: Archive, separatorBefore: true, onClick: () => { archiveBlotter(row.original.id, session?.name ?? "Staff"); toast.success("Case archived.") } },
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deletingId) {
            deleteBlotter(deletingId, session?.name ?? "Staff")
