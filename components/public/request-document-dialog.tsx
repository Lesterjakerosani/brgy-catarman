...
export function RequestDocumentDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const submitPublicRequest = useCertificatesStore((s) => s.submitPublicRequest)
  ...
  function onSubmit(values: RequestValues) {
    ...
    const request = submitPublicRequest({ ...values, requirements })

    console.log("Confirmation email (simulated) sent to", values.email, "for request", request.referenceNumber)
