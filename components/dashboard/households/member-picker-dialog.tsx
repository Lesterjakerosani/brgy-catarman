...
export function MemberPickerDialog({ open, onOpenChange, mode, excludeResidentIds, defaultAddress, onConfirm }: MemberPickerDialogProps) {
  ...
  function handleCreateNew(values: QuickResidentValues) {
    ...
    const resident = addResident(payload, session?.name ?? "Staff")
    toast.success(`${getResidentFullName(resident)} added as a new resident.`)
