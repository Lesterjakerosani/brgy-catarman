...
import { PLACEHOLDER_GROUPS } from "@/lib/certificate-placeholders"
...
interface CertificateDocumentEditorProps {
  html: string
  onChange: (html: string) => void
}

export function CertificateDocumentEditor({ html, onChange }: CertificateDocumentEditorProps) {
  const editableRef = React.useRef<HTMLDivElement>(null)

  // Only sync from props into the DOM when the template actually changes
  // (e.g. switching templates), never on every keystroke -- otherwise the
  // cursor would jump back to the start of the document on every input.
  React.useEffect(() => {
  ...
  return (
    <div className="rounded-xl border border-border bg-white shadow-sm">
      <div className="sticky top-16 z-20 overflow-x-auto rounded-t-xl border-b border-border bg-white">
        <div className="flex min-w-max items-center gap-0.5 p-2">
          ...
          <Button
            ...
            size="icon"
            className={ICON_BUTTON}
            onClick={() => insertPlaceholder("captain_signature")}
          ...
          <Button
            ...
            size="icon"
            className={ICON_BUTTON}
            onClick={() => insertPlaceholder("resident_photo")}
            aria-label="Insert resident photo"
            title="Insert Resident Photo"
          >
      ...
      <div className="overflow-x-auto rounded-b-xl bg-secondary/30 p-4 sm:p-8">
        <div
          ref={editableRef}
          contentEditable
          ...
          onInput={emitChange}
          className={cn(
            "mx-auto min-h-[297mm] w-[210mm] max-w-none bg-white p-[18mm] text-[13px] leading-relaxed text-[#0F172A] shadow-lg outline-none",
            "cert-document"
          )}
