...
export function CertificateActionDialog({ open, onOpenChange, action, request }: CertificateActionDialogProps) {
  ...
  function handleConfirm() {
    ...
    if (!request) return
    if (action === "extend") {
      updateStatus(request!.id, request!.status, actor, { extendDays: Number(extendDays) || 0 })
      toast.success(`Claim deadline extended by ${extendDays} day(s).`)
    } else {
      updateStatus(request!.id, meta.status!, actor, {
        rejectionReason: action === "reject" ? reason : undefined,
        staffNotes: notes || undefined,
      })
      toast.success(`Request marked as "${meta.status}".`)
    }
    onOpenChange(false)
  }
