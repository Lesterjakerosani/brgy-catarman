...
export function TrackRequestDialog({
  ...
  const { request: certRequest, isLoading: certLoading } = useTrackCertificateRequest(searchedRef || undefined)
  const { complaint, isLoading: complaintLoading } = useTrackComplaint(certRequest ? undefined : searchedRef || undefined)
  const anyLoading = certLoading || complaintLoading

  const result = React.useMemo<
    { type: "certificate"; data: CertificateRequest } | { type: "complaint"; data: Complaint } | null
  >(() => {
    if (certRequest) return { type: "certificate", data: certRequest }
    if (complaint) return { type: "complaint", data: complaint }
    return null
  }, [certRequest, complaint])
