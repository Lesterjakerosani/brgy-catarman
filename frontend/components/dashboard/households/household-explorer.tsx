...
interface HouseholdExplorerProps {
  ...
  onAddPurok?: () => void
  onEditPurok?: (purokId: string) => void
  onDeletePurok?: (purokId: string) => void
}
...
export function HouseholdExplorer({
  ...
  const { puroks: allPuroks } = useSitios()
  ...
  return (
    <div className="space-y-3">
      ...
      {hasActiveFilter && totalMatching === 0 ? (
        <p className="rounded-[12px] border border-dashed border-border bg-card px-4 py-6 text-center text-sm text-muted-foreground">
      ...
      >
        {purokRows.map(({ purok, households, residentCount }) => (
          <AccordionItem
            key={purok.id}
            value={purok.id}
            className="rounded-[12px] border border-border bg-card shadow-[0_2px_6px_rgba(15,23,42,0.05)] transition-shadow duration-150 hover:shadow-[0_4px_10px_rgba(15,23,42,0.08)]"
          >
            <div className="flex items-center gap-1 px-2">
              <AccordionTrigger className="flex-1 items-center px-2 py-3 hover:no-underline">
                <span className="flex items-center gap-3">
                  <Folder className="size-5 shrink-0 text-muted-foreground" />
                  <span className="flex flex-col gap-0.5 text-left">
                    <span className="text-sm font-semibold text-foreground">{purok.name}</span>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="size-8 shrink-0 rounded-[10px] text-muted-foreground hover:bg-secondary"
                        aria-label="Purok actions"
                      >
                        <MoreHorizontal className="size-4" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-40">
                      <DropdownMenuItem onClick={() => onEditPurok?.(purok.id)}>
                        <Pencil className="size-4" />
                        Edit
                      </DropdownMenuItem>
                      <DropdownMenuItem variant="destructive" onClick={() => onDeletePurok?.(purok.id)}>
                ...
                >
                  {households.map(({ household, headName, status }) => (
                    <AccordionItem
                      key={household.id}
                      value={household.id}
                      className="rounded-[10px] border border-border bg-card transition-colors duration-150 hover:bg-muted/40"
                    >
                      <div className="flex items-center gap-1 px-2">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-52">
                              ...
                              {household.isArchived ? (
                                <DropdownMenuItem
                                  onClick={() => {
                                    restoreHousehold.mutate(household.id)
                                  }}
                                >
                                ...
                                <DropdownMenuItem
                                  onClick={() => {
                                    archiveHousehold.mutate(household.id)
                                  }}
                                >
                      ...
                      <AccordionContent className="border-t border-border px-1">
